            ...
            if ($reqBody['criterio'] == 'niu') {
                if (isset($reqBody['dato_busqueda'])) {
                    if ($reqBody['dato_busqueda'] != "") {
                        ...
                        $response['indisponibilidades_actuales'] = $api->wsIndisponibilidadesBpmco($niu, false);
                        /* $aniomes = $api->consultargetSQSQuery("SELECT MAX(ANIOMES) as aniomes FROM dw.DATOSBASICOS ");
                        $aniomes = $aniomes["aniomes"];
                        $anio = $api->consultargetSQSQuery("SELECT YEAR(getdate()) as anio");
                        $anio = $anio["anio"]; */
                            /*$response['anio']= $anio;
                            $response['aniomes']=$aniomes;*/
                        /* $response['historico'] = $api->consultargetSSCountHistoricoIndis($niu, $aniomes, $anio);
                        $response['historico'] = $response['historico']["countHistorico"]; */
                        $response['historico'] = 11;
