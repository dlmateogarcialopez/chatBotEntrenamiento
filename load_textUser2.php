...
$data = $api->chargeText2();
// var_dump($data);
if (is_array($data)) {
    if (count($data) > 0) {        
        loadReportsTexts2();
        $datos = cargaArchivo($data, 'ARCHIVO_TEXTOS_2.xls', $api);
    }
}
...
{  
    ...
    /*foreach ($data as $key2 => $rep2) {
        $prueba = strlen($rep2->mensaje[0]->text);
        $prueba3 = date('Y-m-d h:i:s', $rep2->mensaje[0]->fecha / 1000);
        $prueba4 = $rep2->mensaje[0]->codUser;
        $prueba2 = '';
    }*/
    ...
    try {
        foreach ($data as $key => $rep) {
            $objPHPExcel->getActiveSheet()->setCellValue('A' . $i, $rep->mensaje[0]->text);
            $objPHPExcel->getActiveSheet()->setCellValue('B' . $i, date('Y-m-d h:i:s', $rep->mensaje[0]->fecha / 1000));
            if(isset($rep->mensaje[0]->codUser)){
                $objPHPExcel->getActiveSheet()->setCellValue('C' . $i, $rep->mensaje[0]->codUser);
            }
            if (strlen($rep->mensaje[0]->text)>= 50) {
                $objPHPExcel->getActiveSheet()->setCellValue('D' . $i, 'Mayor');
            }else{
                $objPHPExcel->getActiveSheet()->setCellValue('D' . $i, 'Menor');
            }
